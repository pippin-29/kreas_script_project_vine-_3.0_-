...
function brush(dim) {
  ...
  Emitter.prototype = {
    ...
    start: function(event, mode4) {
      if (this.starting)
        this.starting = false, this.emit("start", event, mode4);
      else
    ...
    brush: function(event, mode4) {
      ...
      if (--this.active === 0)
        delete this.state.emitter, this.emit("end", event, mode4);
      return this;
    },
...
function quickselect3(array6, k2, left2 = 0, right2 = Infinity, compare) {
  ...
  while (right2 > left2) {
    ...
    if (compare(array6[left2], t) === 0)
      swap3(array6, left2, j);
    else
      ++j, swap3(array6, j, right2);
    if (j <= k2)
      left2 = j + 1;
    if (k2 <= j)
      right2 = j - 1;
...
function ticker(year2, month2, week, day2, hour2, minute2) {
  ...
  function tickInterval(start2, stop, count5) {
    ...
    if (i === tickIntervals.length)
      return year2.every(tickStep3(start2 / durationYear, stop / durationYear, count5));
    if (i === 0)
...
function ticker2(year2, month2, week, day2, hour2, minute2) {
  ...
  function tickInterval(start2, stop, count5) {
    ...
    if (i === tickIntervals.length)
      return year2.every(tickStep(start2 / durationYear2, stop / durationYear2, count5));
    if (i === 0)
      return millisecond3.every(Math.max(tickStep(start2, stop, count5), 1));
    const [t, step] = tickIntervals[target / tickIntervals[i - 1][2] < tickIntervals[i][2] / target ? i - 1 : i];
    return t.every(step);
  }
  return [ticks4, tickInterval];
